...
########################################
# Build PokerTH QML Client
########################################

BUILD_DIR="$SCRIPT_DIR/$BUILD_DIR_NAME"

log "Configuring CMake build (QML Client)…"
rm -rf "$BUILD_DIR"
mkdir -p "$BUILD_DIR"
...
cmake -S "$SCRIPT_DIR" -B "$BUILD_DIR" \
  ...
  -DOPENSSL_ROOT_DIR="$VCPKG_DIR/installed/$VCPKG_TRIPLET"
...
# USE_QML=1 → passes -qmldir="src/gui/qt6-qml" to macdeployqt
build_bundle_and_dmg "$BUILD_DIR" "$BUILD_TARGET" "1" "$SCRIPT_DIR"
