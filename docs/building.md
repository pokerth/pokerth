...
flowchart TB
  ...
  subgraph dock[Build using docker]
    ...
    DG --> Dbld[docker/Dockerfile → docker build]
...
- **Dev Container:** Open the **repo root**, use `.devcontainer/devcontainer.json` (**Reopen in Container**). In the integrated terminal, run `make windows` / `make android` (not `make *-docker` — the Makefile blocks that). The image sets `IN_DOCKER=1` (see `docker/Dockerfile`); `containerEnv` sets `IN_DEVCONTAINER=1`. `CACHE_ROOT` is `/opt/pokerth` in the container (`scripts/functions.sh`), with the host’s `~/.pokerth` bind-mounted there — same files as native, different absolute path prefix than host CMake (see **building-developer.md**). Details, mounts, and **ensure** vs setup manifest: **building-developer.md**.

### Defaults, setup manifest, and clean
...
- **Setup manifest:** When you run `make linux`, `make windows`, or `make macos`, Make checks for `BUILD_DIR/.manifest.env` (e.g. `build_linux/.manifest.env`, `build_windows/.manifest.env`, `build_macos/.manifest.env`; Docker Windows uses `docker/windows/build/.manifest.env`). If it is missing, Make runs the corresponding `setup.sh` first (`setup_*.sh` writes the manifest), then builds. So `make` or `make linux` (etc.) can be run without having run setup beforehand.
- `make clean`: Removes `build_linux/`, `build_windows/`, `build_macos/`, `build_android/`, `docker/windows/build/`, `docker/android/build/` (including each `/.manifest.env`). After that, the next `make` or `make linux` (etc.) will run setup again.
...
| **Android** | Host: **make setup-android** (**VCPKG_DIR**) provisions SDK/NDK/Gradle/Qt + vcpkg; Docker on macOS | `make android` or `make android-docker` | `build_android/` (host) or `docker/android/build/` (Docker `IN_DOCKER=1`) | Linux: **build_android.sh** via **build.sh**. macOS: **make android** uses Docker. Image: `docker/Dockerfile` `final` runs `make setup-toolchains` (SDK/NDK/Gradle + toolchain **manifest**), same pattern as Windows MinGW. Qt (**aqt**), vcpkg ports, and protobuf overlays run at `docker run` when **ensure** invokes `setup.sh deps`. |

- **Clean rebuild:** `make clean` (removes build dirs and `/.manifest.env`) or `CLEAN=yes make linux` (or make windows / make macos) wipes the build dir and reconfigures from scratch. If you see "Makefile not found", "Configure incomplete", or cache/toolchain errors, run `CLEAN=yes make <target>` and retry.
...
1. **Setup (once):** `make setup-linux` or `make setup` (on Linux). Or run `./scripts/setup.sh` (dispatcher → **setup_linux.sh**; may need sudo). If you skip this, `make linux` or `make` will run setup automatically (writes `build_linux/.manifest.env` when done).
...
**NSIS (makensis):** Required only for `make windows-installer`. On Linux run `make setup-windows` (installs `nsis`), or `apt install nsis`. In the unified devcontainer image, the **base** **apt** merge includes `windows-apt-packages.txt` (**nsis** is listed there). `setup_linux.sh` also runs during `docker build` for Linux native deps; `make windows` / `setup.sh` may invoke **apt** again—usually a no-op. **build_linux.sh** does not install **nsis**; it errors clearly if `makensis` is missing.
...
Same `run_devcontainer.py` plan and host caches as `make windows-docker`: open the **repository root** in Cursor/VS Code and use `.devcontainer/devcontainer.json` (image `docker/Dockerfile`). Repo at `/workspaces/pokerth`; the image sets `IN_DOCKER=1`, so `scripts/functions.sh` sets `VCPKG_DIR` / `QT_OUTPUT_DIR` under `docker/windows/build/`. `ensure_docker_deps.py` may run `setup.sh deps` before the inner `make windows`. The `Makefile` passes `REPO_BUILD_ROOT` into `build.sh` (`build_linux.sh`), so binaries land in `docker/windows/build/deploy/`. On failure, check `docker/windows/build/vcpkg/buildtrees/` logs. Prerequisites: [README_windows.md](../docker/windows/README_windows.md). Mounts, **ensure**, and setup manifest: **building-developer.md**.
...
1. **Setup (once):** `make setup-macos` or `make setup` (on macOS). Or run `./scripts/setup_macos.sh`. Requires Xcode and Xcode command line tools. If you skip this, `make macos` or `make` will run setup automatically (writes `build_macos/.manifest.env` when done).
