...
**Where outputs live on disk:** Non-Docker builds use **`build_<platform>/`**. **`make *-docker`** and devcontainers use **`docker/<kind>/build/`** on the host (including **`…/build/vcpkg`**). The **Makefile** (**`STAMP_DIRS`**, **`REPO_BUILD_ROOT`**) defines the same layout as **`make help`**; longer notes: **docs/building-developer.md**.
...
1. **On Linux:** **Setup (once):** `make setup-windows` (or `TARGET_PLATFORM=windows ./scripts/setup.sh`, which runs **setup_linux.sh**). If you skip this, **`make windows`** will run setup automatically (creates `build_windows/.stamp_setup` when done).
...
**Troubleshooting:** See **docs/windows_troubleshooting.md** (e.g. 0xc0000022, DEP, antivirus, missing DLLs). For build failures on Linux before copying, see the **Linux** section above. **Docker:** Host **`docker/windows/build/vcpkg`** and **`docker/windows/build/Qt`** mount to **`/opt/pokerth-windows/vcpkg`** and **`/opt/pokerth-windows/Qt`** (populated at **`docker run`** via **ensure** / **setup.sh deps**, not baked in **`docker build`**). First container run: **ensure_docker_deps.py** may run **setup.sh deps** when vcpkg or Qt tooling is not ready. Logs: **`docker/windows/build/vcpkg/buildtrees/`** if setup fails. More on Docker flows: **docs/building-developer.md**.

### Building in Dev Container (Windows)

Open the **repository root** in Cursor/VS Code and use the Windows devcontainer (**docker/windows/.devcontainer/**); the repo root is mounted at `/workspaces/pokerth`. The devcontainer flow matches **`make windows-docker`** (same config from `devcontainer.json` via **scripts/run_devcontainer.py**): host **`docker/windows/build/vcpkg`** and **`docker/windows/build/Qt`** mount to **`/opt/pokerth-windows/vcpkg`** and **`/opt/pokerth-windows/Qt`**. In Docker, **ensure_docker_deps.py** sets **`IN_DOCKER=1`**; the **Makefile** computes **`REPO_BUILD_ROOT`** and passes it to **build.sh** (**build_linux.sh**), so output is in **`docker/windows/build/deploy/`**. See **docker/windows/README_windows.md** for prerequisites.
...
5. **Other architectures:** **`make android ANDROID_BUILD_ARGS="--arch x86_64"`** or match **`VCPKG_TRIPLET`** to the ABI when using **setup-android**. See **docker/android/README_android.md**.
...
- **Setup scripts:** **setup.sh** dispatches: **setup_linux.sh** (linux/windows host), **setup_android.sh**, **setup_macos.sh**. **Build scripts:** **build.sh** dispatches: **build_linux.sh**, **build_android.sh**, **build_macos.sh**. **make clean:** Removes build dirs and stamps.
- **vcpkg:** Host default **~/vcpkg** (**VCPKG_DIR**). Docker **run**: **`docker/windows/build/vcpkg`**, **`docker/android/build/vcpkg`**. **scripts/ensure_docker_deps.py** runs before **`make`** in **`*-docker`** flows when **vcpkg** ports or **Qt** (**`qt-cmake`** under the mounted Qt tree) are not ready, then **`setup.sh deps`**. Details: **docs/building-developer.md**.
