...
IMAGE_NAME="pokerth-windows-qml-builder:latest"
NO_CACHE="${1:-}"

echo "=== PokerTH QML-Client Windows-Installer – Docker-Build ==="
...
docker build \
    ${NO_CACHE:+--no-cache} \
    -f "$SCRIPT_DIR/Dockerfile.windows-qml" \
    -t "$IMAGE_NAME" \
    "$PROJECT_ROOT"
...
echo ""
echo "=== Fertig! ==="
INSTALLER=$(ls "$SCRIPT_DIR"/PokerTH-QML-*-Setup.exe 2>/dev/null | tail -1)
if [ -n "$INSTALLER" ]; then
    ls -lh "$INSTALLER"
    echo ""
