...
public:
	...
	void setHandNum(uint32_t num)
	{
		handNum_ = num;
	}
	uint32_t mySet() const
	{
		return mySet_;
	}
	void setMySet(uint32_t val)
	{
		mySet_ = val;
	}
	...
	void setHighestSet(uint32_t val)
	{
		highestSet_ = val;
	}
	uint32_t myCash() const
	{
		return myCash_;
	}
	void setMyCash(uint32_t val)
	{
		myCash_ = val;
	}
	NetGameState lastGameState() const
	{
		return lastGameState_;
	}
	void setLastGameState(NetGameState state)
	{
		lastGameState_ = state;
	}
	NetGameState currentGameState() const
	{
		return currentGameState_;
	}
	void setCurrentGameState(NetGameState state)
	{
	...
	try {
		...
		cout << "       Waiting for response (3s timeout)..." << endl;
		{
			...
			if (!reply) {
				...
				case CleanerChatReplyMessage_CleanerActionType_cleanerActionNone:
					actionStr = "NONE";
					break;
				case CleanerChatReplyMessage_CleanerActionType_cleanerActionWarning:
					actionStr = "WARNING";
					break;
				case CleanerChatReplyMessage_CleanerActionType_cleanerActionKick:
					actionStr = "KICK";
					break;
				case CleanerChatReplyMessage_CleanerActionType_cleanerActionBan:
					actionStr = "BAN";
					break;
				case CleanerChatReplyMessage_CleanerActionType_cleanerActionMute:
					actionStr = "MUTE";
					break;
				default:
					actionStr = "UNKNOWN(" + to_string(r.cleaneractiontype()) + ")";
					break;
				}
