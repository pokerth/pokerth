...
ClientStateWaitSession::InternalHandlePacket(boost::shared_ptr<ClientThread> client, boost::shared_ptr<NetPacket> tmpPacket)
{
	if (tmpPacket->GetMsg()->messagetype() == PokerTHMessage::Type_InitAckMessage) {
		...
		if (!client->GetContext().GetPassword().empty()) {
			{ time_t _t = time(NULL); char _ts[32]; strftime(_ts, sizeof(_ts), "[%Y-%m-%d %H:%M:%S] ", localtime(&_t)); std::cout << _ts << "[BBCBot] Session established, downloading and loading bot files..." << std::endl; }
