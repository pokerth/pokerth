...
ServerLobbyThread::HandleNetPacketChatRequest(boost::shared_ptr<SessionData> session, const ChatRequestMessage &chatRequest)
{
	...
	if (session->GetPlayerData() && session->GetPlayerData()->GetRights() != PLAYER_RIGHTS_GUEST) {
		if (!chatRequest.has_targetgameid() && !chatRequest.has_targetplayerid()) {
          ...
          if((session->GetPlayerData()->GetDBId() == 36 || session->GetPlayerData()->GetDBId() == 37 || session->GetPlayerData()->GetDBId() == 45 || session->GetPlayerData()->GetDBId() == 73) && (targetSession->GetPlayerData()->GetDBId() == 338 || targetSession->GetPlayerData()->GetDBId() == 36 || targetSession->GetPlayerData()->GetDBId() == 37 || targetSession->GetPlayerData()->GetDBId() == 45 || targetSession->GetPlayerData()->GetDBId() == 73)){
