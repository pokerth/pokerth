...
struct String {
	...
	{
		try {
			return s.empty() ? 0u : static_cast<unsigned int>(std::stoul(s));
		} catch(...) {
			return 0u;
		}
	}
...
public:
	...
	Query &operator<<(const Quote &)
	{
		m_quoteNext = true;
		return *this;
	}
	template<typename T>
	Query &operator<<(const T &v)
	{
